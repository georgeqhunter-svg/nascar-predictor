# backtest_oddslogic.py — compare our model vs OddsLogic closing lines for
# two real 2026 races.
#
#   1. 2026-08-29 Coke Zero Sugar 400 at Daytona (superspeedway)
#   2. 2026-09-06 Cook Out Southern 500 at Darlington (intermediate)
#
# For each, retrain model on data BEFORE that race, predict, score each
# matchup vs actual outcome.
...
def run_race(target_date, name_match, matchups, races, entries, sessions, loopstats, laptimes):
    ...
    if daytona.empty:
        ...
        return None
    target_rid = daytona.iloc[0]["race_id_short"]
    target_date_ts = daytona.iloc[0]["date"]
    tt = daytona.iloc[0]["track_type"]

    print(f"\n{'=' * 70}")
    print(f"{daytona.iloc[0]['race_name']} at {daytona.iloc[0]['track_name']} "
          f"({tt}), race_id={target_rid}, date={target_date_ts}")
    print(f"{'=' * 70}")

    features = build_features(races, entries, sessions, loopstats=loopstats, laptimes=laptimes)
    features["date"] = pd.to_datetime(features["date"])
    train = features[
        (features["date"] < target_date_ts) & (features["finish_pos"] > 0)
    ]
    target = features[features["race_id_short"] == target_rid].reset_index(drop=True)
    if target.empty:
        print(f"no feature rows for {target_rid}")
        return None

    print(f"Training on {train['race_id_short'].nunique()} races, predicting {len(target)} entries.")
    model = GBMEnsemble()
    ...
    print(f"T for {tt} = {T} (all types: {T_by_type})")
    ...
    for a, b, oa, ob in matchups:
        if a not in driver_to_idx or b not in driver_to_idx:
            print(f"  skip {a} vs {b}: not in field")
            continue
        if a not in actual.index or b not in actual.index:
            print(f"  skip {a} vs {b}: no result")
            continue

        # Market probs (vig-removed)
        ma_raw = american_to_prob(oa)
        mb_raw = american_to_prob(ob)
        vig = ma_raw + mb_raw
        ...
        # Outcome
        fa = int(actual.loc[a, "finish_pos"])
        fb = int(actual.loc[b, "finish_pos"])
        a_won = fa < fb
        ...
        rows.append({
            "matchup": f"{a[:15]} vs {b[:15]}",
            "actual": a if a_won else b,
            "mkt_A%": round(ma * 100, 1),
            "mdl_A%": round(p_model_a * 100, 1),
            "mkt_LL": round(-_log_clip(p_market_won), 3),
            "mdl_LL": round(-_log_clip(p_model_won), 3),
        })
    ...
    n = len(rows)
    mkt_mean = float(np.mean(market_ll))
    mdl_mean = float(np.mean(model_ll))
    print(f"\nTotals ({n} matchups):")
    print(f"  Market log-loss: {mkt_mean:.4f}   correct: {market_correct}/{n} ({market_correct/n*100:.1f}%)")
    print(f"  Model log-loss:  {mdl_mean:.4f}   correct: {model_correct}/{n} ({model_correct/n*100:.1f}%)")
    delta = mdl_mean - mkt_mean
    winner = "MODEL WINS" if delta < 0 else "MARKET WINS"
    print(f"  Δ (model - market): {delta:+.4f}  → {winner}")
    return {"race": name_match, "n": n, "market_ll": mkt_mean, "model_ll": mdl_mean,
...
def main():
    ...
    if len(summary) > 1:
        combined_mkt = np.mean([s["market_ll"] for s in summary])
        combined_mdl = np.mean([s["model_ll"] for s in summary])
        print(f"\nCombined market log-loss: {combined_mkt:.4f}")
        print(f"Combined model log-loss:  {combined_mdl:.4f}")
        print(f"Δ: {combined_mdl - combined_mkt:+.4f}")
