# backtest_oddslogic_v4.py — five-race comparison vs OddsLogic closing lines.
import numpy as np
import pandas as pd
...
def run_race(target_date, name_match, matchups, races, entries, sessions, loopstats, laptimes):
    ...
    print(f"{name_match} ({tt}, T={T}): n={n}  mkt={mkt_mean:.4f}({market_correct}/{n})  "
          f"mdl={mdl_mean:.4f}({model_correct}/{n})  Δ={mdl_mean-mkt_mean:+.4f}")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("SUMMARY (five races vs OddsLogic closing lines)")
    print("=" * 70)
