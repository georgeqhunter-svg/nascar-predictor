# backtest_oddslogic_v2.py — three-race comparison vs OddsLogic closing lines.
#   1. 2026-08-23 Dollar Tree 301 at New Hampshire (short)  — 35 matchups
#   2. 2026-08-29 Coke Zero Sugar 400 at Daytona (superspeedway) — 30 matchups
#   3. 2026-09-06 Cook Out Southern 500 at Darlington (intermediate) — 30 matchups

import numpy as np
import pandas as pd

# ------------------------------------------------------------------------- #
# Matchups. Odds are from Circa where available, else BetOnline (New Hampshire
# late matchups). Format: (A, B, oddsA, oddsB).
# ------------------------------------------------------------------------- #
LOUDON = [
...
def main():
    ...
    print("\n" + "=" * 70)
    print("SUMMARY (three races vs OddsLogic closing lines)")
    print("=" * 70)
    print(pd.DataFrame(summary).round(4).to_string(index=False))
    total_mkt = np.mean([s["market_ll"] for s in summary])
    total_mdl = np.mean([s["model_ll"] for s in summary])
