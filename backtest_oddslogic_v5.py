...
def run_race(target_date, name_match, matchups, races, entries, sessions,
    ...
    # -------- Per-track-type temperature calibration (robust to sample size) --------
    from src.models.matchup_calibrate import (
        apply_matchup_temperature, fit_matchup_temperature,
    )
    cal_probs_by_type: dict[str, list] = {}
    cal_outcomes_by_type: dict[str, list] = {}
    for vr, v_tt in zip(val_races, tt_list):
        ...
        cal_outcomes_by_type.setdefault(v_tt, [])
        for ii in range(n_v):
            for jj in range(ii + 1, n_v):
                if f[ii] <= 0 or f[jj] <= 0: continue
                if f[ii] == f[jj]: continue
                cal_probs_by_type[v_tt].append(float(m_v[ii, jj]))
                cal_outcomes_by_type[v_tt].append(int(f[ii] < f[jj]))
    all_probs = [p for lst in cal_probs_by_type.values() for p in lst]
    all_out = [o for lst in cal_outcomes_by_type.values() for o in lst]
    T_match_global = (fit_matchup_temperature(np.asarray(all_probs),
                                              np.asarray(all_out))
    ...
    if cal_probs_by_type.get(tt) and len(cal_probs_by_type[tt]) >= 200:
        T_match = fit_matchup_temperature(
            ...
            np.asarray(cal_outcomes_by_type[tt]),
        )
    else:
        T_match = T_match_global

    # Unified sampling at T * T_match — one distribution for all products.
    ...
    print(f"{name_match} ({tt}, T={T}, Tm={T_match:.2f}): n={n}  mkt={mkt_mean:.4f}({market_correct}/{n})  "
